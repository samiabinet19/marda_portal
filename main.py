...
@web_app.route('/')
def home():
  return 'Marda Portal Bot is running perfectly!', 200
...
async def check_expired_payments_logic(bot):
  approved_users = await asyncio.to_thread(
      lambda: list(users_col.find({'payment_status': 'ፅድቋል (Approved)'}))
  )
  today = datetime.now()
      ...
      try:
        ...
        if days_passed == 28:
          await bot.send_message(
              ...
              text=(
                  f'⚠️ <b>የክፍያ ማስታወሻ!</b>\n\nሰላም <b>{name}</b>፣ ክፍያዎ'
                  ' ለማለቅ <b>2 ቀን ብቻ</b> ቀርቶታል። አገልግሎቱ እንዳይቋረጥብዎ ያድሱ።'
              ),
          ...
          await bot.send_message(
              chat_id=uid,
              text=(
                  f'🔔 <b>የክፍያ ጊዜዎ አብቅቷል!</b>\n\nሰላም <b>{name}</b>፣ 30 ቀናት'
                  " ስለሞሉ የወሩ ክፍያ ጊዜዎ አብቅቷል። በ <b>'💳 ክፍያ ፈፅም'</b> በኩል ድጋሚ"
                  ' ይክፈሉ።'
              ),
              parse_mode=ParseMode.HTML,
          )
...
def main_menu(user_id: int) -> InlineKeyboardMarkup:
  ...
  keyboard = [
      [
          InlineKeyboardButton(
              '🏫 ስለ ትምህርት ቤቱ (School Info)', callback_data='school_info'
          )
      ],
      [
          InlineKeyboardButton(
              '📝 አዲስ ምዝገባ (Register)', callback_data='register'
          )
      ],
  ...
  if user and user['payment_status'] == 'ፅድቋል (Approved)':
    keyboard.append(
        [InlineKeyboardButton('🌟 VIP ቻናል መግቢያ', url=VIP_LINK)]
    )

  keyboard.append(
      [InlineKeyboardButton('📞 ግንኙነት (Contact)', callback_data='contact')]
  )

  if is_admin(user_id):
    keyboard.append([
        InlineKeyboardButton(
            '⚙️ የአድሚን ገጽ (Admin)', callback_data='admin_panel'
        )
    ])
...
def get_batches_keyboard() -> InlineKeyboardMarkup:
  ...
  for b in range(15, 51):
    b_name = f'{b}ኛ ባች'
    row.append(
        InlineKeyboardButton(f'{b}ኛ', callback_data=f'reg_batch_{b_name}')
    )
    if len(row) == 3:
      keyboard.append(row)
      row = []
...
def back_menu() -> InlineKeyboardMarkup:
  return InlineKeyboardMarkup(
      [[InlineKeyboardButton('⬅️ ወደ ዋና ማውጫ ተመለስ', callback_data='main')]]
  )
  ...
  if user and user['is_banned'] == 1:
    if update.message:
      await update.message.reply_text(
          '❌ <b>እርስዎ ከዚህ ቦት ታግደዋል!</b>', parse_mode=ParseMode.HTML
      )
    elif update.callback_query:
      try:
        await update.callback_query.answer(
            '❌ እርስዎ ከዚህ ቦት ታግደዋል!', show_alert=True
        )
      except Exception:
        pass
    return True
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  await update.message.reply_text(
      'በጣም ጥሩ! አሁን <b>የስልክ ቁጥርዎን</b> ያስገቡ፡', parse_mode=ParseMode.HTML
  )
  return REG_PHONE
...
async def get_batch(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  await query.edit_message_text(
      ...
      " <b>'💳 ክፍያ ፈፅም'</b> የሚለውን በመጫን ደረሰኝ ማስገባት ይችላሉ።",
...
async def start_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  await query.edit_message_text(
      payment_info, parse_mode=ParseMode.HTML, reply_markup=back_menu()
  )
  return PAY_RECEIPT
...
async def receive_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  await update.message.reply_text(
      '✅ <b>ደረሰኝዎ ደርሶናል!</b>\nአድሚኑ አረጋግጦ እስኪያፀድቀው ድረስ እባክዎን ትንሽ'
      ' ይታገሱ።',
      reply_markup=main_menu(user_id),
      parse_mode=ParseMode.HTML,
  )
...
async def handle_admin_action(
  ...
  if action == 'approve':
    ...
    vip_markup = InlineKeyboardMarkup(
        [[InlineKeyboardButton('🌟 ወደ VIP ቻናል ይግቡ', url=VIP_LINK)]]
    )
    await context.bot.send_message(
        chat_id=target_user_id,
        text=(
            '🎉 <b>እንኳን ደስ አለዎት!</b> ክፍያዎ በአድሚኑ ተረጋግጦ ፅድቋል። አሁን VIP'
            ' ቻናላችንን መቀላቀል ይችላሉ!'
        ),
    ...
    await context.bot.send_message(
        chat_id=target_user_id,
        text=(
            '❌ <b>ክፍያዎ አልፀደቀም።</b> እባክዎን ትክክለኛ ደረሰኝ መላክዎን'
            ' ያረጋግጡ።'
        ),
        parse_mode=ParseMode.HTML,
    )
...
async def show_admin_panel(query, context):
  ...
  paid_users = await asyncio.to_thread(get_paid_users_only)

  report = (
      f'⚙️ <b>የአድሚን መቆጣጠሪያ Dashboard</b>\n\n👥 <b>ጠቅላላ ተጠቃሚዎች:</b>'
      f' {len(all_users)}\n✅ <b>ከፍለው ደረሰኝ ያፀደቁ:</b> {len(paid_users)}'
  ...
  admin_buttons = InlineKeyboardMarkup([
      [
          InlineKeyboardButton(
              '✅ ከፍለው ደረሰኝ የላኩ ብቻ (Paid)', callback_data='show_paid_only'
          )
      ],
      [
          InlineKeyboardButton(
              '📁 በባች አስተዳድር/ማስታወቂያ ላክ (Manage Batch)',
              callback_data='show_batch_menu',
          )
      ],
      [
          InlineKeyboardButton(
              '📢 ለአጠቃላይ ተጠቃሚ መልእክት (General Broadcast)',
              callback_data='start_broadcast',
          )
      ],
      [
          InlineKeyboardButton(
              '🔄 ክፍያዎችን በግድ ፈትሽ', callback_data='force_check_payments'
          )
      ],
      [InlineKeyboardButton('⬅️ ወደ ዋና ማውጫ', callback_data='main')],
  ])
  await query.edit_message_text(
      report, reply_markup=admin_buttons, parse_mode=ParseMode.HTML
  )
...
async def show_batch_selector_admin(query, context):
  ...
  for b in range(15, 51):
    b_name = f'{b}ኛ ባች'
    row.append(
        InlineKeyboardButton(f'{b}ኛ', callback_data=f'adm_batch_{b_name}')
    )
    if len(row) == 3:
      keyboard.append(row)
      row = []
...
async def show_batch_options_menu(query, context, batch_name):
  ...
  buttons = InlineKeyboardMarkup([
      ...
      [
          InlineKeyboardButton(
              '⬅️ ወደ ባች መረጣ ተመለስ', callback_data='show_batch_menu'
          )
      ],
  ])
  await query.edit_message_text(
...
async def prompt_batch_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  await query.edit_message_text(
      ...
      ' ተማሪዎች ብቻ እንዲላክ የሚፈልጉትን መልእክት ይፃፉ፦\n\n(ለማቋረጥ /cancel ይበሉ)',
      parse_mode=ParseMode.HTML,
  )
...
async def send_batch_text_message(
  ...
  if not batch_name:
    await update.message.reply_text('❌ የባች መረጃው አልተገኘም። እባክዎን እንደገና ይሞክሩ።')
    return ConversationHandler.END
  ...
  if not users:
    await update.message.reply_text(
        f'❌ በ{batch_name} ውስጥ የተመዘገበ ምንም ተጠቃሚ አልተገኘም።'
    )
    return ConversationHandler.END

  success, failed = 0, 0
  await update.message.reply_text(
      f'⏳ መልእክቱ ለ {batch_name} ተማሪዎች እየተላከ ነው...'
  )
...
async def prompt_batch_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  await query.edit_message_text(
      ...
      f' <b>{batch_name}</b> የሚላከውን PDF ፋይል እዚህ ያያይዙልን (Upload)።\n<i>(ፋይሉ'
      f' በራስ-ሰር በ {batch_name} ፎልደር ውስጥ ይቀመጣል!)</i>\n\n(ለማቋረጥ /cancel'
      ' ይበሉ)',
      parse_mode=ParseMode.HTML,
  )
...
async def send_batch_pdf_document(
  ...
  file_name = (
      doc.file_name
      or f"announcement_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
  )
  file_path = os.path.join(target_folder, file_name)

  await update.message.reply_text(
      f'⏳ ፋይሉ በ {batch_name} ፎልደር ውስጥ እየተቀመጠ እና ለተማሪዎች እየተላከ ነው...'
  )
...
async def show_paid_users_list(query, context):
  ...
  if not paid_users:
    ...
    text = (
        f'✅ <b>ከፍለው ደረሰኝ የላኩ ተጠቃሚዎች ዝርዝር ({len(paid_users)})</b>\n\n'
    )
...
async def revoke_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  try:
    ...
    await update.message.reply_text(
        f'🔄 ተጠቃሚ <b>{user_name}</b> (ID: <code>{target_id}</code>) ክፍያው'
        ' ተሰርዞ ከባቹ ተወግዷል!',
        parse_mode=ParseMode.HTML,
    )
    ...
    try:
      await context.bot.send_message(
          chat_id=target_id,
          text=(
              '⚠️ <b>ማሳሰቢያ፦</b> የክፍያ ማረጋገጫዎ ተሰርዟል! እባክዎን እንደገና ትክክለኛ'
              ' ደረሰኝ ያስገቡ።'
          ),
          parse_mode=ParseMode.HTML,
      )
...
async def ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  try:
    ...
    await update.message.reply_text(
        f'🚫 ተጠቃሚ <b>{user["name"]}</b> (ID: <code>{target_id}</code>) በተሳካ'
        ' ሁኔታ ታግዷል!',
        parse_mode=ParseMode.HTML,
    )
...
async def unban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  try:
    ...
    await update.message.reply_text(
        f'✅ ተጠቃሚ <b>{user["name"]}</b> (ID: <code>{target_id}</code>) ከእገዳ'
        ' ነፃ ወጥቷል!',
        parse_mode=ParseMode.HTML,
    )
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if query.data == 'main':
    ...
    await query.edit_message_text(
        '✅ የክፍያ ማስታወሻዎች በተሳካ ሁኔታ ተላኩ!', reply_markup=back_menu()
    )
    ...
    school_menu = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                'ℹ️ ስለ ትምህርት ቤቱ', callback_data='about_school'
            )
        ],
        [
            InlineKeyboardButton(
                '📚 የሚሰጡ ትምህርቶች', callback_data='school_courses'
            )
        ],
        [
            InlineKeyboardButton(
                '📢 ወቅታዊ ማስታወቂያዎች', callback_data='school_news'
            )
        ],
        [InlineKeyboardButton('⬅️ ወደ ዋና ማውጫ', callback_data='main')],
    ])
    await query.edit_message_text(
        '🏫 <b>የትምህርት ቤቱ መረጃ እና ማስታወቂያዎች</b>\n\nእባክዎን ማወቅ የሚፈልጉትን መረጃ ከታች'
        ' ይምረጡ፡',
    ...
    text = (
        ...
        'ትምህርት ቤታችን በዘመናዊ የትምህርት አሰጣጥ እና በቴክኖሎጂ የተደገፈ ጥራት ያለው ትምህርት ለመስጠት'
        ' የተቋቋመ ነው፡\n\n'
        '🎯 <b>ራዕይ:</b> በደንብ የዳበረ እና ከልምድ ወጥቶ በትምህርት የታገዘ የሳውንድ እውቀት ያለው'
        ' ባለሙያ መፍጠር።\n'
        '⭐ <b>ተልዕኮ:</b> ጥራት ያለውና ተመጣጣኝ ትምህርት ለሁሉም ማዳረስ።'
    )
    sub_menu = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            '⬅️ ወደ ትምህርት ቤት ማውጫ', callback_data='school_info'
        )
    ]])
    await query.edit_message_text(
        text, reply_markup=sub_menu, parse_mode=ParseMode.HTML
    )

  elif query.data == 'school_courses':
    text = (
        '📚 <b>የሚሰጡ ትምህርቶች እና ኮርሶች</b>\n\n'
        '1. 💻 <b>Audio Fundamentals & Sound Physics:</b> የድምፅ ሞገድ ባህሪያት'
        ' (Frequency, Amplitude, Phase)፣ የሰው ልጅ የመስማት ሂደት (Psychoacoustics) እና'
        ' የክፍል አካውስቲክስ (Room Acoustics) መሠረታዊ ሕጎችን ይሸፍናል\n'
        '2. 🇬🇧 <b>የDigital Audio Workstations (DAW) & Signal Flow:</b> እንደ Pro'
        ' Tools, Logic Pro ወይም Ableton ያሉ ሶፍትዌሮችን አጠቃቀም፣ የማይክሮፎን አይነቶችንና'
        ' አቀማመጥ፣ እንዲሁም የኦዲዮ ሲግናል ፍሰትን (Signal Routing) ያስተምራል።\n'
        '3. 📐 <b>የMixing & Mastering Engineering:</b> የተለያዩ የተቀረፁ የድምፅ'
        ' መስመሮችን (Multi-track audio) አዋህዶ ሚዛናዊ ማድረግ (Mixing) እና ለመጨረሻው'
        ' ዲጂታል ስርጭት ጥራቱን ጠብቆ ማዘጋጀትን (Mastering) ያተኩራል'
    )
    sub_menu = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            '⬅️ ወደ ትምህርት ቤት ማውጫ', callback_data='school_info'
    ...
    user = await asyncio.to_thread(get_user, user_id)
    p_date = (
        user['payment_date'] if user and user['payment_date'] else 'ያልተመዘገበ'
    )
    profile_text = (
        '📋 <b>የመገለጫ መረጃ</b>\n\n'
        f'• <b>ስም:</b> {user["name"] if user else "አልተመዘገበም"}\n'
        f'• <b>ስልክ:</b> {user["phone"] if user else "አልተመዘገበም"}\n'
        f'• <b>ባች:</b> {user["batch"] if user else "ያልተመረጠ"}\n'
        f'• <b>የክፍያ ሁኔታ:</b> {user["payment_status"] if user else "አልተከፈለም"}\n'
        f'• <b>መጨረሻ የተከፈለው:</b> {p_date}\n'
        f'• <b>የአካውንት ባላንስ:</b> {user["balance"] if user else 0.0} ETB'
    )
    await query.edit_message_text(
        profile_text, reply_markup=back_menu(), parse_mode=ParseMode.HTML
    )
    ...
    wallet_text = (
        '💰 <b>የእርስዎ የሂሳብ ባላንስ (Wallet)</b>\n\n'
        f'• <b>ያለዎት ባላንስ:</b> <code>{balance} ETB</code>\n\n'
        "💡 ባላንስዎን ለመጨመር በ '💳 ክፍያ ፈፅም' በኩል ደረሰኝ ያስገቡ።"
    )
    await query.edit_message_text(
        wallet_text, reply_markup=back_menu(), parse_mode=ParseMode.HTML
    )
    ...
    contact_text = (
        ...
        '• <b>አድራሻ:</b> አዲስ አበባ ልዩ ስሙ መገናኛ ከዘፍነሽ ሞል ፊት ለፊት ያለው አቢስንያ ባንክ'
        ' የሚገኝበት ላይ ሁለተኛ ፎቅ፣ ኢትዮጵያ'
    )
    await query.edit_message_text(
        contact_text, reply_markup=back_menu(), parse_mode=ParseMode.HTML
    )
    ...
    if is_admin(user_id):
      ...
      await query.edit_message_text(
          '❌ ይህንን ገጽ ለማየት ፈቃድ የሎትም።', reply_markup=back_menu()
      )
...
if __name__ == '__main__':
  # 1. Web Server በ Background Thread ማስጀመር (Flask)
  ...
  pay_handler = ConversationHandler(
      ...
      states={
          PAY_RECEIPT: [
              ...
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, invalid_receipt_format
              ),
          ],
      },
      fallbacks=[
  ...
  batch_announcement_handler = ConversationHandler(
      ...
      states={
          BATCH_MSG_STATE: [
              MessageHandler(
                  filters.TEXT & ~filters.COMMAND, send_batch_text_message
              )
          ],
          BATCH_PDF_STATE: [
              MessageHandler(
                  filters.Document.ALL, send_batch_pdf_document
              )
          ],
      },
      fallbacks=[
  ...
  app.add_handler(
      CallbackQueryHandler(handle_admin_action, pattern='^(approve|reject)_')
  )
